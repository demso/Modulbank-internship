...
Этот проект представляет собой микросервис "Bank Accounts" (Bank Acconts Service) для розничного банка. Он предоставляет API для управления банковскими счетами, транзакциями и выписками.
...
> - Пересоберите образы контейнеров с помощью выполненной в корне проекта команды:
>```
> docker-compose build
>```
